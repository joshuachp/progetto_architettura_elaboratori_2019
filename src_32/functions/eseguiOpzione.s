...
eseguiOpzione_insert_success1:
    ...
    mov     %eax, %edx
    mov     -0xb(%ebp), %esi
    lea     STRINGA_POSIZIONE_VALORE(%eip), %edi
    xor     %eax, %eax
    call    printf@plt
    jmp     eseguiOpzione_switch_4_endif
eseguiOpzione_switch_4_if:
    mov     -0xb(%ebp), %esi
    lea     STRINGA_VALORE_NON_TROVATO(%eip), %edi
    call    printf@plt
...
eseguiOpzione_switch_5:
    ...
    call    calcolaMax
    mov     %eax, %esi
    lea     STRINGA_VALORE_MAX(%eip), %edi
    xor     %eax, %eax
    call    printf@plt
    jmp     eseguiOpzione_exit
eseguiOpzione_switch_6:
    ...
    add     $1, %eax
    mov     %eax, %esi
    lea     STRINGA_POSIZIONE_MAX(%eip), %edi
    xor     %eax, %eax
    call    printf@plt
    jmp     eseguiOpzione_exit
eseguiOpzione_switch_7:
    ...
    call    calcolaMin
    mov     %eax, %esi
    lea     STRINGA_VALORE_MIN(%eip), %edi
    xor     %eax, %eax
    call    printf@plt
    jmp     eseguiOpzione_exit
eseguiOpzione_switch_8:
    ...
    add     $1, %eax
    mov     %eax, %esi
    lea     STRINGA_POSIZIONE_MIN(%eip), %edi
    xor     %eax, %eax
    call    printf@plt
    jmp     eseguiOpzione_exit
eseguiOpzione_switch_9:
    ...
    mov     %eax, %esi
    lea     STRINGA_VALORE_FREQ(%eip), %edi
    xor     %eax, %eax
    call    printf@plt
    jmp     eseguiOpzione_exit
...
eseguiOpzione_switch_default:
    lea     STRINGA_OPT_NON_SUPPORTATA(%eip), %edi
    xor     %eax, %eax
    call    puts@plt
eseguiOpzione_exit:
    mov     %ebp, %esp
