...
/* Questo programma fornisce una semplice interfaccia utente per operare su un
   vettore di interi. L'utente inserisce il vettore di interi all'avvio
   dell'applicazione e poi opera su di esso per mezzo di un menu' testuale. */
...
int calcolaMediaIntera(void) {
    ...
    for (i = 0; i < LUNGHEZZA_VETTORE; i++) somma += vettore[i];
